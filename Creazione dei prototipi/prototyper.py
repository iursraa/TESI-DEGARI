...
def insertArtworkInDict(instance):
    ...
    open((path + arttwork.replace("'", "_") + ".txt"), "w").close
...
if __name__ == "__main__":
    ...
    for arttwork in dict:
        # conto le parole totali dell'artwork
        totWords = sum(dict[arttwork].values())

        # Calcolo min e max delle medie
        minFreq = 1
        maxFreq = 0
        for word in dict[arttwork]:
            freq = dict[arttwork][word] / totWords
        ...
        filename = path + arttwork.replace("'", "_") + ".txt"
        file = open(filename, "w", encoding=encoding)

        # Scrivo su file le parole dell'artwork.
        for word, count in sorted(dict[arttwork].items(), key=lambda kv: kv[1], reverse=True):
            freq = count / totWords
